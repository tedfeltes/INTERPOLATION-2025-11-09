...
def test_commands_and_alias(stripped: str) -> None:
    ...
    assert "zhover--rel-box" in names
...
def test_hover_loop_and_magenta(lsp_text: str) -> None:
    ...
    assert "EntityTransparency" in lsp_text
    assert '"25"' in lsp_text or "33554457" in lsp_text
    assert "(cons 51 15.0)" in lsp_text
    assert "(rtos z 2 2)" in lsp_text
...
def test_format_is_digits_only(lsp_text: str) -> None:
    assert "(rtos z 2 2)" in lsp_text
